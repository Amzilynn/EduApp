...
def main():
    ...
    missing_phrases = {
        "أين_الدائرة_الحمراء؟.wav": "أَيْنَ الدَّائِرَةُ الْحَمْرَاءُ؟",
        "أين_المثلث_الأصفر؟.wav": "أَيْنَ الْمُثَلَّثُ الْأَصْفَرُ؟"
    }

    for filename, text in missing_phrases.items():
        wav_path = boy_dir / filename
        try:
            synthesize(model, text, wav_path, str(boy_ref))
        except Exception as e:
            print(f"Error: {e}")

    print("\nDone generating missing combination phrases for Boy.")
