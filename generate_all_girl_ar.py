"""
Generate ALL Arabic girl voice recordings from manifest.json
Reference voice: girlarb.mp4  (extracted → 22050Hz mono WAV, normalized)
Output:          public/recordings/ar/girl/<file>   (WAV, matching manifest)
...
Run inside Docker:
  docker run --rm --dns 8.8.8.8 \
    ...
    -w /mnt/EDUAPP xtts-tester python generate_all_girl_ar.py
"""
...
girl_items = manifest["recordings"]["ar"]["girl"]
print(f"Step 2: Loaded manifest — {len(girl_items)} Arabic girl entries to generate\n")
...
for i, item in enumerate(girl_items, 1):
...
print()
print("=" * 60)
print(f"DONE — Arabic girl voice generation complete!")
